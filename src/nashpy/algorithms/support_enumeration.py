...
def solve_indifference(
    A: np.ndarray, rows: np.ndarray = None, columns: Iterable[Optional[int]] = None
) -> bool:
    """
...
def potential_support_pairs(
    A: np.ndarray, B: np.array, non_degenerate: bool = False
...
def indifference_strategies(
    A: np.ndarray, B: np.ndarray, non_degenerate: bool = False, tol: float = 10 ** -16
...
def obey_support(
    strategy: np.ndarray, support: np.ndarray, tol: float = 10 ** -16
) -> bool:
    """
...
def is_ne(
    strategy_pair: Tuple[np.ndarray, np.ndarray],
    support_pair: Tuple[np.ndarray, np.ndarray],
