...
from nashpy.integer_pivoting.integer_pivoting import (make_tableau,
                                                      find_pivot_row,
                                                      non_basic_variables,
                                                      pivot_tableau)
...
class TestPolytope(unittest.TestCase):
    ...
    def test_particular_pivot(self):
        ...
        next_tableau = np.array([[  3.,   3.,   1.,   0.,   0.,   1.],
                                 [  0.,   9.,  -2.,   3.,   0.,   1.],
                                 [  0.,  18.,   0.,   0.,   3.,   3.]])
        self.assertTrue(np.array_equal(tableau, next_tableau),
                        msg="{} != {}".format(tableau, next_tableau))
        self.assertEqual(pivot_tableau(tableau, column_index=2), set([0]))
        next_tableau = np.array(np.array([[3.,   3.,   1.,   0.,   0.,   1.],
                                          [6.,  15.,   0.,   3.,   0.,   3.],
                                          [0.,  18.,   0.,   0.,   3.,   3.]]))
        self.assertTrue(np.array_equal(tableau, next_tableau),
                        msg="{} != {}".format(tableau, next_tableau))
